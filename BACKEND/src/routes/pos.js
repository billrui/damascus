...
const createSaleSchema = z.object({
  ...
  open_invoice_id: z.union([z.coerce.number().int(), z.null()]).optional(), // settled invoice (mark paid + broadcast)
});
...
router.post('/sales', requirePermission('pos'), strictLimiter, validate(createSaleSchema), async (req, res, next) => {
  try {
    ...
    // Settle the open invoice this sale paid off: mark the open-invoice row paid
    // and tell every terminal (drops it from open lists, flashes PAID on the
    // waiter board). Best-effort — never blocks the completed sale.
    if (open_invoice_id) {
      console.log('💰 PAID EMIT → invoice', open_invoice_id, 'table', completeSale.table_no || table_no);
      try {
        await db.query(
          `UPDATE hold_orders SET status = 'cancelled', notes = 'paid-invoice'
           WHERE id = $1 AND notes = 'open-invoice'`,
          [open_invoice_id]
        );
      } catch (e) { console.error('Mark invoice paid failed:', e.message); }
      emitInvoicePaid({
        id:       open_invoice_id,
        table_no: completeSale.table_no || table_no || null,
        ts:       new Date().toISOString(),
      });
    }
...
router.post('/invoices', requirePermission('pos'), async (req, res, next) => {
  try {
    const { table_no, items, total, discount_pct = 0, customer, source_hold_ids } = req.body;
    const holdRef = (Array.isArray(source_hold_ids) && source_hold_ids.length)
      ? source_hold_ids.join(',')
      : `INV-OPEN-${Date.now()}`;

...
export default function WaiterPOS({ user, menuItems: propMenuItems, holdList, setHoldList, openInvoices, setOpenInvoices }) {
  ...
  const mkBill = (persons, val = (parseFloat(hhPct) || 0), type = hhType) => {
    ...
    return {
      ...
      items, holdIds, subtotal: sub,
      discount, hhType: type, hhPct: v,
      total: net*(1+TAX+SVC),
    };
  };
  ...
  const recompose = (sel, mode, persons, val, type) => {
    const list = persons || (billTarget ? billTarget.persons : []);
    const v    = val  != null ? val  : (parseFloat(hhPct) || 0);
    const t    = type != null ? type : hhType;
    const chosen = list.filter(x => sel.includes(x.person));
    setComposedBills(mode === "together"
      ? (chosen.length ? [mkBill(chosen, v, t)] : [])
      : chosen.map(x => mkBill([x], v, t)));
  };

  const setHhValAnd = (v) => {
    const n = Math.max(0, parseFloat(v) || 0);
    setHhPct(n);
    recompose(customSel, billMode, null, n, hhType);
  };
  const setHhTypeAnd = (t) => {
    setHhType(t);
    recompose(customSel, billMode, null, parseFloat(hhPct) || 0, t);
  };
    ...
    try {
      ...
      for (const bill of composedBills) {
        const inv = await posApi.createInvoice({
          ...
          items:    bill.items,
          total:    bill.total,
          notes:    bill.personIds.join("+") + " — " + (tbl==="TAKEAWAY" ? "Takeaway" : "Table " + tbl) + (bill.discount>0 ? ` — Happy Hour ${bill.hhType==="percent" ? bill.hhPct+"%" : "KES "+bill.hhPct} (-KES ${bill.discount})` : ""),
        ...
        if (inv?.id) {
          ...
          setOpenInvoices(p => p.find(x => String(x.id) === String(inv.id))
            ...
            : [{
                ...
                discount: bill.discount, discountType: bill.hhType, discountPct: bill.hhPct,
                total: bill.total, finalTotal: bill.total, holdIds: bill.holdIds,
                  ...
                  <div key={h.id} style={{
                    ...
                    <div style={{
                      ...
                      <div style={{display:"flex",gap:6}}>
                        ...
                        <button onClick={()=>setHeldOrders(p=>p.filter(x=>x.id!==h.id))} style={{
        ...
        <div style={overlay}>
          <div style={{background:T.surface,border:"1px solid "+T.border,borderRadius:14,padding:22,width:420,maxWidth:"92vw",maxHeight:"88vh",display:"flex",flexDirection:"column"}}>
            ...
            <div style={{flex:1,overflowY:"auto",marginBottom:14,borderTop:"1px solid "+T.border,paddingTop:12}}>
              ...
              {composedBills.map((b)=>(
                <div key={b.id} style={{background:T.card,border:"1px solid "+T.border,borderRadius:8,padding:"10px 12px",marginBottom:6}}>
                  ...
                  {b.discount>0 && (
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:10.5,color:T.textMuted,marginTop:4}}>
                      <span>Subtotal {fmt(b.subtotal)} · Happy Hour {b.hhType==="percent" ? b.hhPct+"%" : fmt(b.hhPct)}</span>
                      <span style={{color:T.green,fontWeight:700}}>−{fmt(b.discount)}</span>
                    </div>
                  )}
                </div>
              ))}

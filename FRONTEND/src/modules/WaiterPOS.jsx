...
export default function WaiterPOS({ user, menuItems: propMenuItems, holdList, setHoldList, openInvoices, setOpenInvoices }) {
  ...
  const handleSendKitchen = async () => {
    ...
    // Clear only current person cart
    setCart([]);
  ...
  const handleAddToHold = async hold => {
    ...
    const updItems = [...hold.items, ...newItems];
    const newTotal = updItems.reduce((s,i)=>s+i.price*i.qty,0);
    setHoldList(p=>p.map(h=>h.id===hold.id?{...h,items:updItems,total:newTotal,status:"pending"}:h));
    setCart([]);
    setEditHold(null);
    setModal("kitchen_sent");
    setTimeout(()=>setModal(null),2200);
    try {
      const { posApi } = await import("../api/index.js");
      await posApi.updateHold(hold.id,{items:updItems,total:newTotal,status:"pending"});
    } catch(e) { console.error(e); }
  };
    ...
    <div style={{flex:1,display:"flex",overflow:"hidden",background:T.bg,fontFamily:T.font,color:T.textPrimary}}>
      ...
      <div style={{width:340,display:"flex",flexDirection:"column",background:T.surface,borderRight:"1px solid "+T.border}}>
        ...
        <div style={{flexShrink:0,maxHeight:260,overflowY:"auto",padding:"8px 12px",borderBottom:"1px solid "+T.border}}>
          <div style={{fontSize:9,fontWeight:700,color:T.textMuted,marginBottom:6}}>
            {table==="WALK-IN"?"WALK-IN CUSTOMERS:":"TABLE "+table+" — PERSONS:"}
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:5}}>
            ...
            {/* Add person — only when persons already exist */}
        ...
        <div style={{padding:"6px 12px 4px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            ...
            <span style={{
              ...
              border:"1px solid "+T.amber,background:T.amber,color:T.bg,
            }}>{table==="WALK-IN"?"WALK":table}</span>
            {/* Person pill */}
            <span style={{
              padding:"2px 8px",borderRadius:20,fontSize:10,fontWeight:700,
              border:"1px solid "+T.amber+"55",background:T.amber+"18",color:T.amber,
            }}>{person}</span>
            <span style={{fontSize:9,color:T.textMuted}}>
              {editHold?"ADDING ITEMS":"NEW ORDER"}
            </span>
          </div>
          <div style={{display:"flex",gap:6}}>
            {editHold&&<button onClick={()=>setEditHold(null)} style={{fontSize:10,color:T.red,background:"none",border:"none",cursor:"pointer",fontWeight:700}}>Cancel</button>}
            {!editHold&&cart.length>0&&<button onClick={()=>setCart([])} style={{fontSize:10,color:T.red,background:"none",border:"none",cursor:"pointer",fontWeight:700}}>Clear all</button>}
          </div>
        </div>
